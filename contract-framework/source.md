---
title: "ContractFramework.lua"
parent: Contract Framework
nav_order: 9
---

# ContractFramework.lua
...
wants to read the whole engine start to finish, or diff it against their own copy.

## Deploy

Copy to `scripts/OnLoad/` with a low `[OnLoad]` number so it loads before any contract file — see
[Contract.Register & Lifecycle](register-and-lifecycle#deploying-a-contract-file) and the
[section index](index#load-order) for the full reasoning.
...
- [Units, AI Orders & Relations](units-ai-and-relations) — `C._spawnUnits`, the `AI_BEHAVIORS` table, and
  faction relation save/restore.
- [The Contract Board](contract-board) — the player-facing UI this engine publishes state to via
  `C.Status()`/`C.All()`/`C.onFinish`.
- [WaveDefense](../wave-defense) — a bespoke gamemode built on `onBegin` + `hideTracker`.
