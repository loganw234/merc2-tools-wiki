...
**`def.hideTracker = true` skips building the tracker at all:**
...
This is the board's own half of the same flag [`Contract.Register & Lifecycle`](register-and-lifecycle#handing-off-to-a-bespoke-gamemode)
documents on the framework side (which independently suppresses the native HUD objective-tray line). Set
together, a `hideTracker` contract gets a completely clean screen for a gamemode — like
[WaveDefense](../wave-defense) — that draws its own HUD via [UI Kit](../uilib/) instead.
...
- [WaveDefense](../wave-defense) — a `hideTracker` gamemode built on top of this board.
