...
rather than the ones you'd reach for to move an object or spawn a vehicle. Four source files:
`00_core.lua` (`Ess.Log`, `Ess.Safe`, `Ess.Table`, `Ess.Guid`/`Ess.Name`), `01_math.lua` (`Ess.Math`),
`22_state.lua` (`Ess.State`, `Ess.SaveVar`), and `53_rng.lua` (`Ess.RNG`). `00_core.lua` loads first (the
`00_` prefix is deliberate) and has zero dependencies on the rest of Ess — literally everything else in the
