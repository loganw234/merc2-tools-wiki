...
| `Human.Inventory.ReloadAll` | `Human.Inventory.ReloadAll(uCharGuid, bFlag)` | Confirmed with a character `uGuid` and a boolean in one real call site: `Human.Inventory.ReloadAll(uCharacter, false)` (`vz/xQ!L.lua`). |
