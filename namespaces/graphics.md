...
| `SetShadowBaseDistance` | `Graphics.SetShadowBaseDistance(nDistance)` | Confirmed in `resident/mrxbriefing.lua` (set to `2` during cinematic briefings, restored afterward) and [`vz/xQ!L.lua:582`](../vz/xql) (set to `10`). Single numeric argument, presumably a distance multiplier/scale rather than raw world units given the small values used. |
