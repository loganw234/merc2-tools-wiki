...
| `Flyby` | `uJet = Airstrike.Flyby(vDeliveryVehicle, nStartX, nStartZ, nEndX, nEndZ, nY, nSpeed, [fDropCallback], [tCallbackArgs])` | **CONFIRMED LIVE** via WebSocket lua-bridge probe against a running game (2026-07-22) — not just call-site-inferred. This was already the single most-used call here by call-site count (17 sites, second only to the two spawners), and the live probe now confirms the mechanism itself: it spawns a delivery aircraft that flies from the spawn point to the target point and **returns the vehicle guid**, verified VISIBLE in-game — a B2 and an Autogunship were both observed flying over the player. Confirmed real plane templates: `"Support Vehicle (AC130)"`, `"Support Vehicle (Autogunship)"`, `"Support Vehicle (B2)"`, `"Support Vehicle (F117)"`, `"Support Vehicle (Cruise Missile)"`. It invokes `fDropCallback` at the drop point, where the module then does the actual `SpawnOrdnance`/`ConeSpawn`/`SpawnCarpetBombLine` — the optional callback+data is how B2 bombing runs work in practice (e.g. `..., DropBomb, {self}`). **A real gotcha: watch the argument order** — `(startX, startZ, endX, endZ, Y, speed)` is **X, Z, then Y-as-altitude**, NOT the X, Y, Z order you'd expect; two XZ pairs followed by a single altitude value, not XYZ triples. `vDeliveryVehicle` is either one of the template strings above or a per-instance `self.uDeliveryVehicle` field. Callback + args are optional — `mrxfactionmanager.lua:1552` omits them. Speeds seen 50–200. Call sites incl. `mrxbombingrun.lua:25`, `mrxcarpetbomb.lua:27`, `mrxcombatairpatrol.lua:25`, `mrxcruisemissile.lua:45`, `mrxfuelairbomb.lua:41`. |
| `SpawnPlaneNew` | `Airstrike.SpawnPlaneNew(sName, nNumber, ...)` *(unconfirmed — see Notes)* | **Investigated, inconclusive — not a working plane spawner as currently understood.** Unlike every other row in this table, `SpawnPlaneNew` has zero call sites anywhere in the decompiled corpus; it surfaced from a static `luaL_Reg` engine-binding dump (973 registered bindings total) rather than from grepping usage, and was chased specifically because the name suggested a possible standalone alternative to `Flyby`. Disassembly shows it only reads a string plus one number. Live-probed over WebSocket lua-bridge against a running game (2026-07-22) with both a `Flyby`-style 7-argument call and a simple `(name, x, y, z)` call — **neither spawned anything visible or returned a guid**. This is a dead end as currently understood, not a claim that it's broken or unusable outright: it's either a lower-level primitive something else calls internally, or it needs a signature this investigation didn't try — pinning it down further would need more reverse-engineering or a runtime hook. `Flyby` above remains the confirmed, working mechanism for spawning delivery aircraft. |
