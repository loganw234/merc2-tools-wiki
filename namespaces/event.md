...
| `Minigame` | 37 | `{playerGuid, nTimeOut, sAction, button [, nTime]}` or `{playerGuid, nTimeOut, sAction, buttonA, buttonB}` for `sAction == "alternate"` | **Confirmed.** This is a timed QTE (quick-time-event) evaluator, not a raw input poll — `button`/`buttonA`/`buttonB` are `Controller.*` numeric constants (e.g. `Controller.LStick_Left`, `Controller.RPad_Up`, `Controller.Use_Melee`), and `sAction` is one of `"press"`, `"hold"` (adds a trailing `nTime` — required hold duration), `"tap"`, or `"alternate"` (two buttons, mash-between-them). Confirmed extensively in `resident/mrxactionhijack.lua`'s vehicle-hijack minigame (`Event.Create(Event.Minigame, {self._hijackerPlayer, self[self.nCurrent].miniGame.nTimeOut, self[self.nCurrent].miniGame.sAction, self[self.nCurrent].miniGame.button}, self[self.nCurrent].OnMinigameStatus, {self})` and variants). The callback receives `(self, sStatus, n)` where `sStatus` is `"success"`, `"failed"`, or `"update"` — this reports outcome/progress on the QTE, it does not hand back continuous raw button state. This is the same `Controller.*` constant vocabulary documented on the [Controller](controller) namespace page — that page previously only had confirmed real usage inside this same hijack minigame; this is the mechanism that consumes them.

**Confirmed live by testing**, and important for anyone hoping to use this for continuous input: `"hold"` mode on a stick-direction constant (`Controller.LStick_Left`) does **not** measure sustained hold duration or report live stick position. Live testing showed `n` climbing in increments (~0.3, ~0.6, ~1.0) only while actively wiggling the stick, reaching `"success"` at `n≈1.0` regardless of the `nTime` argument passed — holding the stick steady in one direction produced no `"update"` events at all. This strongly suggests `"hold"` mode accumulates discrete motion/wiggle pulses toward a threshold (the same mechanic `"alternate"` mode clearly uses for its mash-between-two-directions QTE), not a continuous analog read. **There is no confirmed way to read live/continuous controller or stick state anywhere in this event system** — `Event.Minigame`, like `Event.Button`, only reports discrete, one-shot outcomes. |
