...
| Event Type | ID | Args shape | Notes |
|---|---|---|---|
| `Button` | 39 | `{playerOrGuid, buttonName, "press"\|"release"?, bool}` | **Confirmed.** `Event.Create(Event.Button, {uPlayer, "lbutton", "press", true}, TutorialCancel, {self})` (`vz/gurcon003.lua:317`); also `"rtrigger"`, `"cancel"`, `"selection"`, `"lsleft"`, `"lsright"` seen as button names across `resident/mrxguihudmessage.lua` and `vz/meccon001.lua`. Button-name strings appear to be a fixed controller/input vocabulary rather than raw keycodes. |
