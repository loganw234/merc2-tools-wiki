...
| `RealTime` | — | No call sites found in the decompiled corpus. Exists per live enumeration; presumed real-world-clock counterpart to `MainTime`, paired with `RealTimeStamp`, by naming symmetry only — unconfirmed. |
| `Clock` | — | No call sites found in the decompiled corpus. Exists per live enumeration; unconfirmed. |
| `Date` | — | No call sites found in the decompiled corpus. Exists per live enumeration; unconfirmed. |
...
| `MemUsage` | — | No call sites found in the decompiled corpus. Exists per live enumeration; presumed a memory-diagnostics query by naming, unconfirmed — likely useful for debug/diagnostic mod scripts but untested. |
