...
| `SetBriefingInterior` | `Net.SetBriefingInterior([sInteriorName])` | Confirmed with a string argument (`Net.SetBriefingInterior("WifPmcInterior")`, `Net.SetBriefingInterior("MrxHq")`) and with no arguments (`Net.SetBriefingInterior()`) to presumably clear it, in [`vz/wifpmcinterior.lua`](../vz/wifpmcinterior) and `resident/mrxhq.lua`. |
...
| `BeginLayerEventGroup` / `EndLayerEventGroup` | `Net.BeginLayerEventGroup()` / `Net.EndLayerEventGroup()` | Confirmed with no arguments in [`vz/xQ!L.lua`](../vz/xql), both guarded by an existence check (`if Net.BeginLayerEventGroup then ... end`), used to bracket a group of layer-related network events. |
