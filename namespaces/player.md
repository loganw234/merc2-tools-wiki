...
| `GetLocalId` | `n = Player.GetLocalId(idx)` | **Live-confirmed via WebSocket lua-bridge probe (2026-07-22)** — takes an index argument (`idx`) and returns a number, correcting the earlier no-argument guess. Shares this same `idx` argument with `IsJoined` below, `GetProfileCharacter`/`GetProfileUpgrade` (Costumes & Disguise below), and `GetViewport` (Camera & Viewport below); together with `GetMaximumLocalPlayers`/`GetCurrentLocalPlayers` just below, this reads as a co-op/split-screen local-player-slot family (`idx` selects one of up to `GetMaximumLocalPlayers()` local-player slots; 0-based vs. 1-based indexing not confirmed) — a reasonable synthesis from the shared argument shape, not independently confirmed function-by-function. A wrong/out-of-range `idx` is expected to return `nil` silently rather than error, per this engine's general no-error-on-bad-args behavior — worth testing both `0` and `1` as the first slot before assuming either. |
...
| `GetCurrentLocalPlayers` | `n = Player.GetCurrentLocalPlayers()` | **Live-confirmed via WebSocket lua-bridge probe (2026-07-22)** — no arguments, returns a number; live-tested example returned `1` (one local player joined at test time), confirming the presumed local-only counterpart to `GetCurrentPlayers`. |
...
| `GetMaximumLocalPlayers` | `n = Player.GetMaximumLocalPlayers()` | **Live-confirmed via WebSocket lua-bridge probe (2026-07-22)** — no arguments, returns a number; live-tested example returned `2` (this build supports up to 2 local/split-screen players), tested alongside `GetCurrentLocalPlayers` returning `1`. |
| `IsJoined` | `b = Player.IsJoined(idx)` | **Live-confirmed via WebSocket lua-bridge probe (2026-07-22)** — confirmed boolean return (matching the prior `b =` guess), but the argument is the same local-player-slot `idx` as `GetLocalId` above, not a `uPlayerGuid` as previously guessed. Presumably answers "is a local player joined in slot `idx`." |
...
| `GetViewport` | `nW, nH, nX, nY, ? = Player.GetViewport(idx)` | **Live-confirmed via WebSocket lua-bridge probe (2026-07-22)** — returns 5 values: width, height, x, y, and a 5th value of unconfirmed meaning. Live-tested example returned `2560, 1440, 0, 0, 0` (a 2560×1440 viewport at the origin). The argument is an index (`idx`), the same local-player-slot index as `GetLocalId`/`IsJoined` (Player & Character Identity above) and `GetProfileCharacter`/`GetProfileUpgrade` (Costumes & Disguise below) — corrects the earlier `uPlayerGuid` guess. |
...
| `GetProfileCharacter` | `n = Player.GetProfileCharacter(idx)` | **Live-confirmed via WebSocket lua-bridge probe (2026-07-22)** — takes the same local-player-slot `idx` as `GetLocalId`/`IsJoined` (Player & Character Identity above) and returns a number, correcting the earlier no-argument guess. Presumably the profile's selected character index for local-player slot `idx`, distinct from the no-arg `GetProfileCostume()` above. |
...
| `GetProfileUpgrade` | `n = Player.GetProfileUpgrade(idx)` | **Live-confirmed via WebSocket lua-bridge probe (2026-07-22)** — takes the same local-player-slot `idx` as `GetLocalId`/`IsJoined`/`GetProfileCharacter` above and returns a number, correcting the earlier bare `(...)` placeholder signature. |
...
| `GetPlayerStart` | `v = Player.GetPlayerStart()` | Confirmed with no arguments in real scripts, e.g. `local vSpawnLocation = Player.GetPlayerStart()`, and compared directly to a string constant in [`xQ!L.lua`](../vz/xql) (`Player.GetPlayerStart() ~= "PlayerLocation_Start"`) — suggesting the return value can be a named-location string as well as a position, unconfirmed which form is canonical. |
