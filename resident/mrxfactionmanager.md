...
vs 1.0× at Friendly (via [`GetPriceScale`](#getpricescalessubjectabbrev-sobjectabbrev), which is what
...
- **Changing player reputation with a faction** goes through
  [`SetRelation`](#setrelationssubjectabbrev-sobjectabbrev-nrelation-binitialize) /
  [`ChangeRelation`](#changerelationssubjectabbrev-sobjectabbrev-nrelation) with `sObjectAbbrev == "Pmc"`. Both
