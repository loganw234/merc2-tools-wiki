...
## Overview
A minimal China side job: a generic bounty to destroy every object carrying the `"Allied"` label,
...
## Functions
### `Activated(self)`
Calls the base `MrxTaskJobDestroyType.Activated`, sets the label filter to `"Allied"`, restricts the job
...
- **Mirror pair**: [AllJob003](alljob003) is the exact same file with the label filter flipped to
  `"China"` — each faction offers a side job to hunt down the other faction's labeled assets.
