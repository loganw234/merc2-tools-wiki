...
## Overview
A China outpost-capture side contract wrapping the `ChiJob001_04` outpost location. Same shape as
[ChiCon050](chicon050)/[ChiCon051](chicon051): a static config table plus a one-line VO hook on
activation.
...
## Functions
### `GetOutpostConfig()`
Returns the outpost's static configuration: `sOutpostBldg = "ChiJob001_04_Outpost"`, its single capture
point (`ChiJob001_04_CapturePt1`), the staging/pristine/defense/captured layer names, `sRivalFaction =
...
(`Fiona-In-Mission-Contract-Chi053-01` plus a per-hero follow-up line).
...
- `sRivalFaction = "Vza"`, `nStartingHealth = 6`, `nRusherQuota = 1` match every other outpost file in
  this batch.
- The outpost's own location prefab is named `ChiJob001_04`, distinct from this contract script's own
  `ChiCon053` name.
