...
## Overview
An Allied Nation outpost-capture side contract wrapping the `AllJob001_03` outpost location. Same shape
as [AllCon050](allcon050)/[AllCon053](allcon053): a static config table plus a one-line VO hook on
activation.
...
## Functions
### `GetOutpostConfig()`
Returns the outpost's static configuration: `sOutpostBldg = "AllJob001_03_Outpost"`, its single capture
point (`AllJob001_03_CapturePt1`), the staging/pristine/defense/captured layer names, `sRivalFaction =
...
- The outpost's own location prefab is named `AllJob001_03`, distinct from this contract script's own
  `AllCon052` name.
