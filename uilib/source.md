...
The complete, current source of the engine every other [UI Kit](../uilib/) page documents one widget at a
...
Copy to `scripts/OnLoad/` and give it a low `[OnLoad]` number so it loads before anything built on it — see
[UI Kit's Deploy section](../uilib/#deploy) for the full checklist (movies required, the guard clause every
consumer script should open with).
...
UI.TOAST_X     = UI.TOAST_X or (640 - 320 - 8)   -- = 312 (right-aligned)
...
function UI.Toast(text, opts)
    ...
    if not t then
        t = {}
        t.w = make_widget(UI.FILES.toast, UI.TOAST_X, UI.TOAST_Y + (pick - 1) * 50, 320, 44)
        S.toasts[pick] = t
    end
...
- [UI.Menu](menu) / [UI.List](list) / [UI.Panel, UI.Bar & UI.Toast](panel-bar-toast) /
  [UI.Confirm & UI.Input](confirm-and-input) / [UI.Chat & UI.Board](chat-and-board) — each widget explained
  on its own, with the reasoning behind its design.
- [UI Kit Scripts](scripts) — complete scripts built on this library, including the two demo scripts that
  between them exercise every widget above.
- [WaveDefense](../wave-defense) — a gamemode built on `UI.Menu`'s cycler pattern for its config screen.
