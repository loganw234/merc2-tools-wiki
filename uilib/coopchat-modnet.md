...
The same co-op text chat as [`coopchat.lua`](coopchat), rebuilt on top of [ModNet](../modnet) instead of a
hand-rolled transport — worth reading side by side with the original to see exactly what moving to the
library actually removes.

> **Use this OR `coopchat.lua`, never both at once** — both hijack the identical event id
> (`MrxFactionManager`, `EV_CHAT`/`M.EV = 5`), so running both simultaneously means each would misinterpret
> the other's traffic as its own.

## What changed, concretely

Every piece of the original that was really "the network problem" — the hijack install, the
pack/unpack-bytes encoding, chunking, reassembly, the sender-id scheme — is simply gone from this file.
What's left is close to pure UI glue:

```lua
ModNet.Send(CH, text)                                    -- was: SendChat(text), ~15 lines of packing
ModNet.On(CH, function(sender, text) ... end)             -- was: a hand-rolled NetEventCallback hijack
```

`ModNet.Send`/`On` already handle arbitrary Lua values (strings included), so there's no equivalent of
`packBytes`/`unpackBytes` or the reassembly buffer here at all — that's all inside [ModNet](../modnet) now,
shared with anything else built on it. What stays, because it's genuinely specific to *this* feature, not
the network layer: the [`UI.Chat`](chat-and-board) window itself, the movement-freeze wrap around
`prompt()`/`_endInput()`, and retitling the local echo to `"P<id>: text"`.

## Setup

1. **Both machines**, in order: [`uilib.lua`](../uilib/) (`OnLoad`), then [`ModNet.lua`](../modnet)
   (`OnLoad`) — both before this file.
2. Copy this file to `scripts/OnKey/ModNet_CoopChat.lua`, add `ModNet_CoopChat.lua=f2` under `[OnKey]`.
...
- [`coopchat.lua`](coopchat) — the original, self-contained implementation this one refits onto a shared
  library.
- [ModNet](../modnet) — the transport library itself: the three API layers, the serialization scheme, and
  why channels can now multiplex over one event id safely.
- [UI.Chat / UI.Board](chat-and-board) — the front-end widget, unchanged between both versions.
